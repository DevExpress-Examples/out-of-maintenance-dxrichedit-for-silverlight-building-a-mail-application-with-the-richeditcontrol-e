...
#Region "IUriProvider Members"
        ...
        Public Function CreateImageUri(ByVal rootUri As String, ByVal image As RichEditImage, ByVal relativeUri As String) As String Implements IUriProvider.CreateImageUri
            ...
            Dim imageFormat As RichEditImageFormat = GetActualImageFormat(image.RawFormat)
            Dim data() As Byte = image.GetImageBytes(imageFormat)
            Dim mediaContentType As String = RichEditImage.GetContentType(imageFormat)
            Dim info As New AttachementInfo() With {.Data = data, .MimeType = mediaContentType, .ContentId = imageName}
            Me.Attachments.Add(info)

            Return "cid:" & imageName
        End Function

        Private Function GetActualImageFormat(ByVal imageFormat As RichEditImageFormat) As RichEditImageFormat
            If imageFormat = RichEditImageFormat.Exif OrElse imageFormat = RichEditImageFormat.MemoryBmp Then
